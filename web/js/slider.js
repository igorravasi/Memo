...
function mcImgSlider(k) {
	...
	}, Lb = document, U = window.requestAnimationFrame, db = window.cancelAnimationFrame, kb = [
			"webkit", "ms", "o" ], cb = 0; cb < kb[d] && !U; ++cb) {
	...
	var c = "style", y = "display", G = "visibility", j = "width", z = "height", Y = "top", P = "background", r = "undefined", x = "marginLeft", w = "appendChild", q = "parentNode", o = "nodeName", Q = "innerHTML", X = "offsetWidth", D = setTimeout, L = clearTimeout, F = "indexOf", N = "setAttribute", jb = "removeChild", E = function() {
		...
		if ((c = a[F]("MSIE ")) != -1)
			b = parseInt(a.substring(c + 5, a[F](".", c)));
		if (a[F]("Safari") != -1 && a[F]("Chrome") == -1)
			b = 300;
		if (a[F]("Opera") != -1)
			b = 400;
		return b
	}, bb = sb() < 9, ib = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i
			.test(navigator.userAgent), M = function(a, b) {
		if (a) {
			a.o = b;
			if (bb)
	...
	E.prototype = {
		...
		m : function(h, d, g, c) {
			for (var b = [], j = g - d, k = g > d ? 1 : -1, f = Math
					.ceil(60 * c.c / 1e3), a, e = 1; e <= f; e++) {
				a = d + c.b(e / f, c.d) * j;
				if (h != i)
					a = Math.round(a);
				b.push(a)
	...
	var l = new E, ob = function() {
		...
		a = {
			...
			i : k.autoAdvance,
			j : k.captionOpacity,
			k : k.captionEffect == "none" ? 0 : k.captionEffect == "fade" ? 1
					: 2,
		...
		if (f)
			a.m = Math.ceil(f.offsetHeight * a.g / f[X]);
		qb(k.effect);
		a.n = function() {
			var b;
			if (a.a.p == -1)
		...
		var a = 0, b = 0, c;
		while (a < e.length) {
			c = e[a][g] == "lazyImage" || e[a][h]("data-src")
					|| e[a][g][F](" video") > -1 && typeof McVideo != r;
			if (c) {
				b = 1;
				break
			}
		...
		if (a)
			while (a--)
				u[a][g] = a != b && u[a].on == 0 ? "thumb" : "thumb thumb-on"
	}, Cb = function(a) {
		return a[q][h]("data-autovideo") == "true"
				|| a[h]("data-autovideo") == "true"
	}, Eb = function() {
		var f;
		if (a.l)
			f = T(a.l);
		if (f)
			for (var h = lb(f, "*"), e = 0; e < h[d]; e++)
				h[e][g] == "thumb" && u.push(h[e]);
		var c = u[d];
		if (c) {
			while (c--) {
				u[c].on = 0;
	...
	ub.prototype = {
		...
		f : function() {
			...
			for (var i = [], a = 0; a < b.d; a++)
				i.push("<div rel='" + a + "'>" + (a + 1) + "</div>");
			S[Q] = i.join("");
		...
		g : function() {
			...
			while (a--) {
				if (a == b.a)
					...
					d[a][g] = "";
				if (e[a][o] == "A" || e[a][g] == "video")
					e[a][c][y] = a == b.a ? "block" : "none"
			}
		},
		i : function() {
			O = A("mc-caption");
		...
		e : function(j) {
			var a = e[j], k = 0;
			if (a[o] == "A" && a[g][F]("lazyImage") == -1 || a[o] == "DIV"
					&& a[g] == "video") {
				a = C(a)[0];
				k = 1
			}
			if (a[o] != "IMG") {
				if (a[o] == "A")
					...
					a[h]("data-autovideo")
							&& a[q][N]
									("data-autovideo", a[h]("data-autovideo"));
					this.A(a[q]);
					i = 0
				} else {
					d = a[h]("data-src");
		...
		p : function(i) {
			...
			if (!I) {
				...
				f[w](I);
				if (sb() >= 300)
					f[c].borderRadius = I[c].borderRadius = "0px"
			}
			I[Q] = "";
			var d = i ? i : a.n();
		...
		v : function() {
			...
			return (new Function(
					...
					.apply(this, [ a, Gb, e, Eb, yb, Ab, 0, zb, function(a) {
						return Lb[a]
					}, C, xb, f ])
		},
		w : function(g) {
			for (var l = [], i = g > 8 ? v : Math.round(v / a.f), m = g > 8 ? 1
					: a.f, f = 0; f < m; f++) {
				var k = A("mcSlc"), e = k[c];
				e.left = i * f + "px";
				e[j] = (f == a.f - 1 ? v - i * f : i) + "px";
				e[z] = "0px";
				e[P] = 'url("' + b.e[h]("src") + '") no-repeat -' + f * i
						+ "px 0%";
				if (g == 10)
					e[P] = 'url("' + b.e[h]("src") + '") no-repeat right top';
				else if (g == 12)
					e[P] = 'url("' + b.e[h]("src") + '") no-repeat left bottom';
				e.zIndex = 1;
				e.position = "absolute";
				M(k, 0);
				I[w](k);
		...
		x : function() {
			for (var l = [], k = Math.round(v / a.g), i = Math.round(B / a.m), g = 0; g < a.m; g++)
				for (var f = 0; f < a.g; f++) {
					...
					e[j] = d.w + "px";
					e[z] = d.h + "px";
					e[P] = 'url("' + b.e[h]("src") + '") no-repeat -' + f * k
							+ "px -" + g * i + "px";
					e.zIndex = 1;
					e.position = "absolute";
					M(d, 0);
	...
	var Kb = function() {
		...
		if (++vb < 20)
			if (!m || typeof tooltip == r)
				D(function() {
					nb(e, c)
				}, 300);
			else
				for (var b = C(S), a = 0; a < b[d]; a++)
